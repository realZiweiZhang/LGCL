...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module, clip_model: torch.nn.Module,
    ...
    if class_names:
        class_names = class_names[task_id]
                    
        task_level_text = clip.tokenize(create_text(class_names=class_names))
        task_level_pos_feature = clip.encode_text(task_level_text)
        task_level_neg_feature = None
        if task_id > 1:
            random_neg_id = random.randint(0, task_id)
            task_level_neg_feature = clip.encode_text(clip.tokenize(create_text(class_names=class_names)))

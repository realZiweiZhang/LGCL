...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module, clip_model: torch.nn.Module,
    ...
    original_model.eval()
    clip_model.to(model.device).eval()
    ...
    if class_names:
        ...
        task_level_text = clip.tokenize(create_text(class_names=pos_class_names)).to(mo)
        ...
        with torch.no_grad():
            if original_model is not None:
                ...
                if class_names:
                    ...
                    class_level_pos_feature = clip_model.encode_text(class_level_text)
                    class_level_neg_feature = clip_model.encode_text(clip.tokenize(create_text(class_names=[remove_pos_name[neg_name_id]])))

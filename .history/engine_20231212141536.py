...
def create_name_feature(self, clip_model, name):
    return
def selecte_class_neg_sample(self, clip_model, class_names, task_id):
    assert task_id > 0
    
    valid_class_names = class_names[:task_id]
    flat_class_names = np.array(valid_class_names).flatten()
    random_index = np.random.randint(flat_class_names)
    return 

...
def build_continual_dataloader(args):
    ...
    if args.dataset.startswith('Split-'):
        ...
        splited_dataset, class_mask, class_name = split_single_dataset(dataset_train, dataset_val, args)
    else:
    ...
    for i in range(args.num_tasks):
        if args.dataset.startswith('Split-'):
            ...
            # dataset_train_classes = dataset_train.classes
        ...
        # if dataset_train_classes is not None:
        #     dataloader.append({'train': data_loader_train, 'val': data_loader_val, 'class_name': dataset_train_classes})
        # else:
        dataloader.append({'train': data_loader_train, 'val': data_loader_val})

    return dataloader, class_mask, class_name
...
def split_single_dataset(dataset_train, dataset_val, args):
    ...
    label_names = dataset_train.classes
    split_datasets = list()
    mask = list()
    names = list()
    print('origin label names', label_names)
    if args.shuffle:
        original_labels = labels
        random.shuffle(labels)
        
        shuffled_index = list(map(lambda x:original_labels.index(x)),labels)
        label_names = [label_names[index] for index in shuffled_index]
        print('shuffle', label_names)
    ...
    for _ in range(args.num_tasks):
        ...
        mask.append(scope)
        names.append([label_names[label] for label in scope])
        for k in range(len(dataset_train.targets)):
            if int(dataset_train.targets[k]) in scope:
                train_split_indices.append(k)
                class_names.append(label_names[dataset_train.targets[k]])

        for h in range(len(dataset_val.targets)):
            if int(dataset_val.targets[h]) in scope:
                test_split_indices.append(h)
        
        if args.use_text_encoder class_names:
            subset_train, subset_val =  Subset(dataset_train, train_split_indices), Subset(dataset_val, test_split_indices)
        else:
            subset_train, subset_val = DummyDataset(dataset_train, train_split_indices,class_names), DummyDataset(dataset_val, test_split_indices)
        split_datasets.append([subset_train, subset_val])
    
    return split_datasets, mask, names
...
class DummyDataset(Dataset):
    def __init__(self, dataset, indices, class_names = None):
    ...
    def __getitem__(self, idx):
        ...
        if self.class_names:
            return x, y, self.class_names[idx]
        else:
            return x, y

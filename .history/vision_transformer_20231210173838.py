...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x, task_id=-1, cls_features=None, train=False, 
                         task_level_pos_text=None, task_level_neg_text=None,
                         ):
    ...
    def forward_head(self, res, pre_logits: bool = False, class_level_pos_text=None, class_level_neg_text=None):
        x = res['x']
        
        if class_level_pos_text:
            pos_loss = self.text_supervised_loss(x, class_level_pos_text)
            class_pos_loss = torch.sum(pos_loss) / pos_loss.shape[0]
            
            if class_level_neg_text:
                pos_loss = self.text_supervised_loss(x, class_level_neg_text)
                class_neg_loss = torch.sum(pos_loss) / pos_loss.shape[0]
    ...
    def forward(self, x, task_id=-1, cls_features=None, train=False,
                text_level_pos_text=None,text_level_neg_text=None,
                class_level_pos_text=None, class_level_neg_text=None):
        res = self.forward_features(x, task_id=task_id, cls_features=cls_features, train=train,
                                    text_level_pos_text=text_level_pos_text,text_level_neg_text=text_level_neg_text,)

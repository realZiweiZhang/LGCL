...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module, clip_model: torch.nn.Module,
    ...
    if class_names:
        ...
        task_level_text = clip.tokenize(create_text(class_names=pos_class_names))
        task_level_pos_feature = clip.encode_text(task_level_text)
        task_level_neg_feature = None
        if task_id > 1:
            random_neg_id = random.randint(0, task_id-1)
            task_level_neg_feature = clip_model.encode_text(clip.tokenize(create_text(class_names=class_names[random_neg_id])))
            
    for input, target, name in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        with torch.no_grad():
            if original_model is not None:
                ...
                if class_names:
                    remove_pos_name = pos_class_names.pop(pos_class_names.index(name))
                    neg_name_id = random.randint(0,len(remove_pos_name))
                    
                    class_level_text = clip.tokenize(create_text(class_names=[name]))
                    class_level_pos_feature = clip.encode_text(class_level_text)
                    class_level_neg_feature = clip.encode_text(clip.tokenize(create_text(class_names=[remove_pos_name[neg_name_id]])))
                    
                    # text_inputs = torch.cat([clip.tokenize(f"a photo of a {c}") for c in cifar100.classes]).to(device)
            else:
                cls_features = None
                task_level_pos_feature = None
                task_level_neg_feature = None
                class_level_pos_feature = None
                class_level_neg_feature = None
                
            
        
        output = model(input, task_id=task_id, cls_features=cls_features, train=set_training_mode, 
                       task_level_pos_feature=task_level_pos_feature, task_level_neg_feature=task_level_neg_feature,
                       class_level_pos_feature=class_level_pos_feature, class_level_neg_feature=class_level_neg_feature)

...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module, clip_model: torch.nn.Module,
    ...
    for input, target, name in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        with torch.no_grad():
            if original_model is not None:
                ...
                if class_names:
                    ...
                    task_level_feature = clip.encode_text(task_level_text)
                    
                    class_level_feature = clip.tokenize(create_text(class_names=name))
                    class
        ...
        output = model(input, task_id=task_id, cls_features=cls_features, train=set_training_mode, task_level_text=task_level_text, class_level_text=class_level_text)

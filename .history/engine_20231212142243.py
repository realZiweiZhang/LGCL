...
def create_name_feature(clip_model, name):
    text = clip.tokenize(create_text(class_names=name)).to(clip_model.device)
...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module, clip_model: torch.nn.Module,
    ...
    if args.use_text_encoder:
        ...
        task_level_pos_feature = create_name_feature(clip_model, pos_class_names)
        ...
        with torch.no_grad():
            if original_model is not None:
                ...
                if class_names and name is not None:
                    ...
                    if task_id > 0:
                        class_neg_sample = selecte_class_neg_sample(class_names,task_id)
                        class_level_neg_feature = create_name_feature(clip_model, pos_class_names)
            else:
                cls_features = None

...
def train_one_epoch(model: torch.nn.Module, original_model: torch.nn.Module, clip_model: torch.nn.Module,
    ...
    for input, target, name in metric_logger.log_every(data_loader, args.print_freq, header):
        ...
        with torch.no_grad():
            if original_model is not None:
                ...
                if class_names:
                    ...
                    class_level_neg_feature = clip.encode_text(clip.tokenize(create_text(class_names=remove_pos_name[random_neg_id])))
        ...
        output = model(input, task_id=task_id, cls_features=cls_features, train=set_training_mode, task_level_text=task_level_feature, class_level_text=class_level_feature)
